...
def configure_routes(app: fastapi.FastAPI, files_manager: services.FilesManager):

    @app.get('/video/download')
    def get_download_video(url: str):
        file_path, file_name = files_manager.get_video(url)
    ...
    def get_download_audio(url: str):
        file_path, file_name = files_manager.get_audio(url)
